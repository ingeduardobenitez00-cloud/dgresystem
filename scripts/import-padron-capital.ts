/**
 * @fileOverview Script de importación para padron_capital.dbf
 */
...
async function run() {
  console.log('\n=================================================');
  console.log('   MIGRACIÓN DE PADRON CAPITAL (DBF)');
  console.log('=================================================\n');

  if (!email || !password) {
    console.error('❌ ERROR: Credenciales ADMIN_EMAIL y ADMIN_PASSWORD no encontradas en el archivo .env.');
    process.exit(1);
  }
  ...
  try {
    ...
    const fileName = 'padron_capital.dbf';
    const filePath = path.join(process.cwd(), 'scripts', fileName);
    
    if (fs.existsSync(filePath)) {
      await importFile(fileName, filePath);
    } else {
      console.log(`⚠️ No se encontró el archivo en la ruta: ${filePath}`);
      console.log('Asegúrate de colocar padron_capital.dbf en la carpeta scripts/');
    }
...
async function importFile(fileName: string, filePath: string) {
  ...
  // SheetJS (xlsx) soporta archivos DBF nativamente
